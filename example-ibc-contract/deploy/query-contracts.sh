...
query_contracts_forever() {
    ...
    do
        ...
        result_1=$(secretcli q compute query $(head -n 1 contract-addresses.log) '{"last_ibc_operation":{}}' --node 'tcp://localhost:26657' 2>&1 | jq 2>&1 | fold -w $width)
        #echo "got result: $result_1"
        echo -e "chain 1\n$result_1" > output-query-1.log

        # query chain 2 and save output to a second file
        result_2=$(secretcli q compute query $(tail -n 1 contract-addresses.log) '{"last_ibc_operation":{}}' --node 'tcp://localhost:36657' 2>&1 | jq 2>&1 | fold -w $width)
